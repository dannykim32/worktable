...
const MARKER = "<!-- visual-chat-guidance -->";
...
function usage() {
  return (
    ...
    "  <target-repo>   directory of the project to register visual-chat into\n" +
...
/** Merge the visual-chat entry into <target>/.mcp.json, preserving every other
 *  server. Idempotent: an existing visual-chat entry has its path updated. */
function mergeMcpJson(target, summary) {
  ...
  const existed = Object.prototype.hasOwnProperty.call(servers, "visual-chat");
  servers["visual-chat"] = {
    command: "node",
    args: [distServer],
  };
  ...
  summary.push(
    `${existed ? "updated" : "added"} visual-chat in ${path}\n` +
      `    → node ${distServer}`,
  );
}
...
function guidancePayload() {
  ...
  const endMarker = "<!-- /visual-chat-guidance -->";
