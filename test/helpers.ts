...
export async function spawnServer(
  ...
  const home = opts.home ?? mkdtempSync(join(tmpdir(), "worktable-test-"));
  if (opts.gate) {
    ...
    const stateDir = join(home, ".worktable", deriveWorkspaceId(repoRoot));
  ...
  const client = new Client({ name: "worktable-tests", version: "0.0.0" });
